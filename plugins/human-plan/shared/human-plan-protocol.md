...
- 默认自动 loop：除非进入人工 gate、真实阻塞、流程完成或 `worktree` 隔离准备，当前 skill 完成后必须直接读取下一阶段 skill 的 `SKILL.md`，沿用同一个 Plan Ref 在同一会话继续执行，不要求用户复制下一步命令。
- 自动跳转只允许在 `idea/design/code-scan/batch-code-scan/arch-check -> dev -> plan-check -> design-check -> approval gate`、`approve -> implementation -> audit`、`audit-fixes-required -> audit replan -> plan-check -> design-check -> approval gate` 这些协议路径内发生。
...
- 一个 batch 内只做一次全量扫描，生成候选 Plan 库后逐个推进；当前 batch 未结束前不得重新全量扫描。
- 一个 Plan 到达 `approve`、`confirm`、人类决策或阻塞状态时，写入 `.loop/gates.md` 并继续处理 batch 中下一个可处理 Plan。
- 一个 batch 全部 Plan 进入 `complete`、`gated` 或 `blocked` 后，生成 `.loop/batches/<Batch ID>.md` 归档，再允许下一轮 batch。
- loop 必须维护 `.loop/state.json`、`.loop/batch-index.md`、`.loop/run-log.md`、`.loop/gates.md`、`.loop/changelog.md` 和 `.loop/token-ledger.md`。
- loop 必须尊重 `max_ticks`，达到次数后停止；不得为了完成 batch 而突破 tick 限制。
