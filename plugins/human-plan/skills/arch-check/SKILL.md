...
当前消息没有可用于对应确认项的答复时，不写入任何内容，只展示待确认事项并要求用户在 `/human-plan:arch-check replan <当前 Plan Ref>` 后补充答复。

理解不正确时继续 `/human-plan:arch-check replan <当前 Plan Ref>` 修正待提交 Replan。只有精确的 `/human-plan:arch-check confirm <当前 Plan Ref>` 才能提交；自然语言肯定不算 confirm。未经 confirm，不得进入 `/human-plan:dev`。

## `/human-plan:arch-check`

识别当前业务、产品、代码或架构与成熟方案的差距。Human Plan 只写问题价值、成熟方案启发、目标状态、关键取舍、范围边界和验收结果。设置 Version 为 1、Status 为 `draft`。

只展示短摘要、Status、Needs Reconfirmation、真实 Plan Ref 和下一步命令。若有待确认事项，下一步返回带真实 Plan Ref 的 `/human-plan:arch-check replan`；否则同时说明可 `/human-plan:dev <当前 Plan Ref>` 或继续 `/human-plan:arch-check replan <当前 Plan Ref>`。随后停止。
...
提交待提交 Replan，清除已解决的确认项，增加 Version 并记录变化。仍有未解决项时 Status 设为 `draft`，下一步继续 `/human-plan:arch-check replan <新 Plan Ref>`；全部解决后 Status 设为 `draft`，下一步进入 `/human-plan:dev <新 Plan Ref>`。confirm 只提交 Replan，不代表开发批准。
