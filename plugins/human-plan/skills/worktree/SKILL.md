...
- 从 `Plan ID` 生成 slug：小写，只保留 `a-z`、`0-9`、`-`，合并连续 `-`，去掉首尾 `-`；如果结果为空，使用 `plan-<Plan Ref 的 12 位稳定哈希>`。
...
- 如果当前项目存在 `.claude/skills` 且目标 worktree 缺少 `.claude/skills`，创建指向当前项目 `.claude/skills` 的符号链接；不存在时跳过，不要创建断链。
