...
只展示短摘要、Status、Needs Reconfirmation、真实 Plan Ref 和下一步命令。若有待确认事项，下一步返回带真实 Plan Ref 的 `/idea replan`；否则同时说明可 `/dev <当前 Plan Ref>` 或继续 `/idea replan <当前 Plan Ref>`。随后停止。
